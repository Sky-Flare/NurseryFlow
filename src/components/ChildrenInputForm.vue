    ...
    <form @submit.prevent="submitChildren">
        <div v-for="(day, dayIndex) in days" :key="day">
            ...
            <div v-for="hourIndex in hours" :key="hourIndex">
                <label :for="`children-${dayIndex}-${hourIndex}`">{{ hourIndex + 7 }}h</label>
                <input :id="`children-${dayIndex}-${hourIndex}`" v-model.number="children[dayIndex][hourIndex]" type="number" min="0" />
            </div>
        </div>
...
const children = ref(Array.from({ length: days.length }, () => Array(hours.length).fill(0)));
