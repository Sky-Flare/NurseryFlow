...
import { ArrayElement, Employee, StatusEmployeeOrChild, useEmployeeStore } from '@/store/index';
...
export const useScheduleStore = defineStore('schedule', () => {
    ...
    //         (function (currentIndexSchedule) {
    //             const slots: {
    //                 date: Date;
    //                 children: Hour[];
    //                 employeesNeeded: number;
    //                 employees: typeof employeeToWork;
    //             }[] = getTimeSlot(new Date(new Date(currentDay.date).setHours(7, 30, 0)), 22).map((s) => ({
    //                 date: s,
    //                 children: [],
    //                 employeesNeeded: 0,
    //                 employees: [],
    ...
    //                         (hE) => !hE.daysOff?.includes(key) && hE.countingHours > 0 && !slot.employees.includes(hE) && hE.hoursPerWeek - hE.countingHours < hE.average * (currentIndexSchedule + 1)
    //                     );
    //
    //                     const employeeAlreadyInPreviousSlotAvailable = slots[indexSlot - 1]?.employees?.filter(
    //                         (eAiPs) => eAiPs.countingHours > 0 && !slot.employees.includes(eAiPs) && eAiPs.hoursPerWeek - eAiPs.countingHours < eAiPs.average * (currentIndexSchedule + 1)
    //                     );
