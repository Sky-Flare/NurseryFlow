...
export const useScheduleStore = defineStore('schedule', () => {
    ...
    function generateSchedule(date: Date) {
        ...
        const schdl: Record<Days, ScheduleItem> = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].reduce((acc, day, index) => {
        ...
        const childrenToWork = children.filter((c) => c.status === StatusEmployeeOrChild.WORKING);
        ...
        for (const [key, value] of Object.entries(schdl)) {
            ...
            }[] = getTimeSlot(d, 22).map((s) => ({ date: s, children: [], employeesNeeded: 0, employees: [] }));
            ...
            slots.forEach((slot, indexSlot) => {
                ...
                for (let i: number = 0; i < slot.employeesNeeded; i++) {
                    ...
                    const availableEmployee = employeeToWork.filter(
                        (hE) => !hE.daysOff?.includes(key) && hE.countingHours > 0 && !slot.employees.includes(hE) && hE.hoursPerWeek - hE.countingHours < hE.average * (indexSchedule + 1)
                    );
                    // console.log(
                    //   "availableEmployee",
                    //   JSON.stringify(
                    ...
                    const employeeAlreadyInPreviousSlotAvailable = employeeAlreadyInPreviousSlot?.filter(
                        (eAiPs) =>
                            // {"name":"marine","hoursPerWeek":35,"daysOff":["Wednesday"],"id":1,"status":"working","average":8.75,"countingHours":34.5}

                            eAiPs.countingHours > 0 && !slot.employees.includes(eAiPs) && eAiPs.hoursPerWeek - eAiPs.countingHours < eAiPs.average * (indexSchedule + 1)
                    );
            ...
            slots.forEach((slot, indexSlot) => {
                slot.employees.forEach((employee) => {
                    const currentEmployeeAlreadyOnDay = value.employee?.filter((ce) => ce.id === employee.id);
                    if (currentEmployeeAlreadyOnDay?.length) {
                        ...
                        if (employeeInNextSlot?.length) {
                            ...
                            if (fsdf) {
                                ...
                                fsdf.hours[0].total = (new Date(slots[indexSlot + 1].date).getTime() - fsdf.hours[0].start.getTime()) / (1000 * 60) / 60;
                            }
                        }
                    ...
                    const tSorted = JSON.parse(JSON.stringify(t))
                        .sort((a, b) => {
                            if (!('employee' in a) && b.employee?.hours.length) {
                                return -1;
                            } // a sans employé, b avec employé
                            if (a.employee?.hours.length && !('employee' in b)) {
                                return 1;
                            } // a avec employé, b sans employé
                            if (!('employee' in a) && !('employee' in b)) {
                                return 0;
                            } // les deux sans employé
